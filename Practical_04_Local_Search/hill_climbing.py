...
def simulated_annealing(env, initial_state, initial_temp=100.0, cooling_rate=0.95, min_temp=0.01):
    """
    Simulated Annealing Optimization:
    Probabilistically accepts sub-optimal neighbor moves according to the Metropolis Criterion:
    P = exp(delta_E / T) when delta_E <= 0.
    Allows escaping local optima plateaus dynamically as temperature cools.
    """
    import math
    current_state = initial_state
    current_score = env.objective_function(current_state)
    ...
    while temp > min_temp:
        ...
        neighbor_score = env.objective_function(neighbor)
        delta_e = neighbor_score - current_score

        # Accept move if better, or with Metropolis probability P = exp(delta_E / T) if worse
...
def run_practical_04():
    print("=" * 60)
    print("PRACTICAL 04: LOCAL SEARCH OPTIMIZATION (HILL CLIMBING & ANNEALING)")
    print("=" * 60)
    ...
    # 4. Simulated Annealing to escape local optimum via Metropolis criterion
    random.seed(42)
    print(f"\n[Experiment 4] Simulated Annealing from {sub_optimal_start}:")
    sa_res = simulated_annealing(env, sub_optimal_start)
    print(f"Escaped Local Peak -> Final State: State={sa_res['final_state']}, Score={sa_res['optimal_score']} / 100")
    print(f"Steps Taken: {sa_res['steps_taken']}")

    return res1, res2, rr_res, sa_res
